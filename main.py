...
GC_LINK = os.environ.get(
    "GC_LINK",
    "https://t.me/+0cKdhqQU4p84ZmQ1"
)

GC_CAPTION = os.environ.get(
    "GC_CAPTION",
    "Join gays chatting group 🫠🫠❤️❤️💘"
)

DM_SENT_FILE = os.environ.get(
    "DM_SENT_FILE",
    "dm_sent_users.json"
)

# ---- 2 minute baad wala message ----
TASK_MESSAGE = """📧 Gmail Work • Gmail Tasks

⏱️ 2 Minute Task = ₹10
💰 10 Tasks = ₹100

🔥 Jitne zyada tasks karoge, utni zyada earning hogi!

📩 Message now and start earning! 🚀

@O_oGmailfarmer_bot

@O_oGmailfarmer_bot"""

TASK_DELAY_SECONDS = 120
# =============================================================================
...
MESSAGES = [
    "Hiii muscularr guyy here for friendshipp",
    "hello handsomee toppp versee heree msgg for chattingg"
]
...
async def dm_handler(event):
    ...
    try:
        ...
        print(
            f"DM flow completed for user {sender.id}. "
            f"Task message sent after 2 minutes."
        )

    except Exception as e:
        print(
            f"DM flow failed for user {sender.id}: {e}"
        )
...
async def run():
    ...
    print(
        "DM flow: group link + caption, "
        "then task message after 2 minutes."
    )
