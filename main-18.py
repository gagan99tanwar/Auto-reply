...
# ---- New-user DM welcome feature ----
GC_LINK = os.environ.get("GC_LINK", "https://t.me/+0cKdhqQU4p84ZmQ1")
GC_CAPTION = os.environ.get("GC_CAPTION", "Join gays chatting group 👥")
SEEN_USERS_FILE = os.environ.get("SEEN_USERS_FILE", "seen_users.json")

# ---- Group where the bot should NEVER reply (not even to mentions/hi/hello) ----
IGNORE_GROUP_LINK = os.environ.get("IGNORE_GROUP_LINK", "https://t.me/+0cKdhqQU4p84ZmQ1")
# Optional manual override if you already know the chat ID (skips auto-resolve)
IGNORE_GROUP_ID_OVERRIDE = os.environ.get("IGNORE_GROUP_ID", "")
# =============================================================================
...
ME_ID = None
ME_USERNAME = ""
IGNORE_CHAT_ID = int(IGNORE_GROUP_ID_OVERRIDE) if IGNORE_GROUP_ID_OVERRIDE.strip() else None

# ---- Persistent tracking of users who have already DMed the bot before ----
def extract_invite_hash(link: str):
    m = re.search(r"(?:joinchat/|\+)([\w-]+)", link)
    return m.group(1) if m else None


async def resolve_ignore_chat_id():
    global IGNORE_CHAT_ID
    if IGNORE_CHAT_ID is not None:
        return  # manual override already set, skip auto-resolve
    invite_hash = extract_invite_hash(IGNORE_GROUP_LINK)
    if not invite_hash:
        print(f"Could not parse invite link: {IGNORE_GROUP_LINK}")
        return
    try:
        result = await client(CheckChatInviteRequest(invite_hash))
        if isinstance(result, ChatInviteAlready):
            IGNORE_CHAT_ID = utils.get_peer_id(result.chat)
            print(f"Ignore-group resolved: chat_id={IGNORE_CHAT_ID} (bot will never reply here)")
        else:
            print("Userbot is not a member of the ignore-group yet, so it can't be auto-excluded. "
                  "Join the group first, or set IGNORE_GROUP_ID env var manually.")
    except Exception as e:
        print(f"Could not resolve ignore-group chat id: {e}")
...
def save_seen_users(seen):
    try:
        with open(SEEN_USERS_FILE, "w") as f:
            json.dump(list(seen), f)
    except Exception as e:
        print(f"Could not save seen_users file: {e}")


seen_users = load_seen_users()

WELCOME_GREETINGS = [
    "Hey hru?", "Heyy hru?", "Hey, how are you?", "Hey hru 😊",
    "Heyy, kaise ho? hru", "Hey there, hru?",
]
...
async def handler(event):
    ...
    # ---- completely ignore the excluded group (no replies to anything here) ----
    if IGNORE_CHAT_ID is not None and event.chat_id == IGNORE_CHAT_ID:
        return
    ...
    # ---- new-user DM welcome (runs for private chats regardless of GROUPS_ONLY) ----
    if event.is_private:
        if sender.id not in seen_users:
            seen_users.add(sender.id)
            save_seen_users(seen_users)
            try:
                await event.reply(random.choice(WELCOME_GREETINGS))
                await asyncio.sleep(1.5)
                await client.send_message(event.chat_id, f"{GC_LINK}\n{GC_CAPTION}")
            except Exception as e:
                print(f"Welcome message failed for user {sender.id}: {e}")
            return  # first-time DM handled, don't run trigger logic on this message
